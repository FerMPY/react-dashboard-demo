  ...
  (function(module, exports) {
    "production" !== process.env.NODE_ENV && function() {
      ...
      exports.createElement = function(type2, config, children) {
        if (isValidElementType(type2))
          ...
          if (null === type2) var typeString = "null";
            ...
            isArrayImpl(type2) ? typeString = "array" : void 0 !== type2 && type2.$$typeof === REACT_ELEMENT_TYPE ? (typeString = "<" + (getComponentNameFromType(type2.type) || "Unknown") + " />", i = " Did you accidentally export a JSX literal instead of a component?") : typeString = typeof type2;
          console.error(
      ...
      exports.memo = function(type2, compare) {
        isValidElementType(type2) || console.error(
...
function flattenRoutes(routes, branches = [], parentsMeta = [], parentPath = "") {
  ...
  routes.forEach((route2, index) => {
    var _a;
    if (route2.path === "" || !((_a = route2.path) == null ? void 0 : _a.includes("?"))) {
...
function createStaticHandler(routes, opts) {
  ...
  async function query2(request2, {
    requestContext,
    ...
    if (!isValidMethod(method) && method !== "HEAD") {
      ...
      let { matches: methodNotAllowedMatches, route: route2 } = getShortCircuitMatches(dataRoutes);
      return {
        basename,
        location,
      ...
      let { matches: notFoundMatches, route: route2 } = getShortCircuitMatches(dataRoutes);
      return {
        basename,
        location,
  ...
  async function callDataStrategy(type2, request2, matchesToLoad, matches, isRouteRequest, requestContext, dataStrategy) {
...
async function callDataStrategyImpl(dataStrategyImpl, type2, state, request2, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2, requestContext) {
  ...
  let dsMatches = matches.map((match, i) => {
    ...
    let resolve = async (handlerOverride) => {
      if (handlerOverride && request2.method === "GET" && (match.route.lazy || match.route.loader)) {
        shouldLoad = true;
      }
...
async function callLoaderOrAction(type2, request2, match, loadRoutePromise, handlerOverride, staticContext) {
  let result;
  let onReject;
...
function normalizeRelativeRoutingRedirectResponse(response2, request2, routeId, matches, basename) {
...
function getShortCircuitMatches(routes) {
  let route2 = routes.length === 1 ? routes[0] : routes.find((r) => r.index || !r.path || r.path === "/") || {
...
function mapRouteProperties(route2) {
  let updates = {
    ...
    hasErrorBoundary: route2.hasErrorBoundary || route2.ErrorBoundary != null || route2.errorElement != null
  };
  if (route2.Component) {
    {
      if (route2.element) {
  ...
  if (route2.HydrateFallback) {
    ...
    Object.assign(updates, {
      hydrateFallbackElement: reactExports.createElement(route2.HydrateFallback),
...
function getKeyedLinksForMatches(matches, routeModules, manifest) {
  let descriptors = matches.map((match) => {
    ...
    return [
      route2 && route2.css ? route2.css.map((href) => ({ rel: "stylesheet", href })) : [],
...
function getRouteComponents(route2, routeModule, isSpaMode) {
  ...
  let HydrateFallback = routeModule.HydrateFallback && (!isSpaMode || route2.id === "root") ? routeModule.HydrateFallback : route2.id === "root" ? RemixRootDefaultHydrateFallback : void 0;
  let ErrorBoundary = routeModule.ErrorBoundary ? routeModule.ErrorBoundary : route2.id === "root" ? () => /* @__PURE__ */ reactExports.createElement(RemixRootDefaultErrorBoundary, { error: useRouteError() }) : void 0;
...
function shouldHydrateRouteLoader(route2, routeModule, isSpaMode) {
  return isSpaMode && route2.id !== "root" || routeModule.clientLoader != null && (routeModule.clientLoader.hydrate === true || route2.hasLoader !== true);
}
...
function ServerRouter({
  ...
  for (let match of context.staticHandlerContext.matches) {
    ...
    if (route2 && manifestRoute && shouldHydrateRouteLoader(manifestRoute, route2, context.isSpaMode) && (route2.HydrateFallback || !manifestRoute.hasLoader)) {
...
function createStaticHandlerDataRoutes(manifest, future, parentId = "", routesByParentId = groupRoutesByParentId2(manifest)) {
  return (routesByParentId[parentId] || []).map((route2) => {
    let commonRoute = {
      ...
      hasErrorBoundary: route2.id === "root" || route2.module.ErrorBoundary != null,
      id: route2.id,
      path: route2.path,
      // Need to use RR's version in the param typed here to permit the optional
      // context even though we know it'll always be provided in remix
      loader: route2.module.loader ? async (args) => {
      ...
      action: route2.module.action ? (args) => callRouteHandler(route2.module.action, args) : void 0,
...
function derive(build, mode) {
  ...
  let errorHandler = build.entry.module.handleError || ((error, { request: request2 }) => {
...
async function handleSingleFetchRequest(serverMode, build, staticHandler, request2, handlerUrl, loadContext, handleError) {
  let { result, headers, status } = request2.method !== "GET" ? await singleFetchAction(
    build,
    serverMode,
    staticHandler,
    request2,
...
function unwrapResponse(response2) {
  ...
  return contentType2 && /\bapplication\/json\b/.test(contentType2) ? response2.body == null ? null : response2.json() : response2.text();
}
/**
...
function createRequestHandler({
  ...
  return async (req, res, next) => {
    try {
      ...
      let loadContext = await (getLoadContext == null ? void 0 : getLoadContext(req, res));
...
function createRemixRequest(req, res) {
  var _a, _b;
  let [, hostnamePort] = ((_a = req.get("X-Forwarded-Host")) == null ? void 0 : _a.split(":")) ?? [];
  let [, hostPort] = ((_b = req.get("host")) == null ? void 0 : _b.split(":")) ?? [];
...
function requireContentType() {
  ...
  var QESC_REGEXP = /\\([\u000b\u0020-\u00ff])/g;
  var QUOTE_REGEXP = /([\\"])/g;
  var TYPE_REGEXP2 = /^[!#$%&'*+.^_`|~0-9A-Za-z-]+\/[!#$%&'*+.^_`|~0-9A-Za-z-]+$/;
...
function requireStreams() {
  ...
  var Buffer2 = require$$0$7.Buffer, Transform = require$$1$2.Transform;
...
function requireStringify() {
  ...
  var sentinel = {};
  var stringify2 = function stringify3(object, prefix, generateArrayPrefix, commaRoundTrip, allowEmptyArrays, strictNullHandling, skipNulls, encodeDotInKeys, encoder, filter, sort, allowDots, serializeDate, format, formatter, encodeValuesOnly, charset2, sideChannel2) {
    var obj = object;
