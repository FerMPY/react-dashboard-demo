  ...
  return (
    ...
    (H.cloneElement = function (m, C, N) {
      ...
      if (W === 1) $.children = N;
        ...
        for (var ve = Array(W), ee = 0; ee < W; ee++)
...
function La() {
  ...
  function u(h, y, w) {
    ...
    if (w.expires) {
      if (!g(w.expires) || !Number.isFinite(w.expires.valueOf()))
...
function Pa(e, t, r, n = {}) {
  ...
  let b = {
    ...
    listen(x) {
      ...
      return (
        a.addEventListener(Or, g),
        (s = x),
        () => {
          a.removeEventListener(Or, g), (s = null);
        }
      );
    },
    createHref(x) {
...
function Ct(e, t, r = [], n = {}) {
  return e.map((a, o) => {
    ...
    if (
      ...
      return (
        (n[l] = s), a.children && (s.children = Ct(a.children, t, i, n)), s
      );
    }
  });
}
...
function ja(e, t, r = !1) {
  ...
  for (let l = 0; l < n.length; ++l) {
    ...
    Object.assign(a, g.params),
      i.push({
        params: a,
        pathname: we([o, g.pathname]),
        pathnameBase: Ya(we([o, g.pathnameBase])),
        route: h,
      }),
      g.pathnameBase !== "/" && (o = we([o, g.pathnameBase]));
  }
  return i;
}
function Pt(e, t) {
  ...
  let o = a[0],
    ...
    l = a.slice(1);
  return {
    params: n.reduce((u, { paramName: p, isOptional: g }, h) => {
      if (p === "*") {
        let w = l[h] || "";
...
function Bi(e) {
  ...
  if (E)
    if (E.some((c) => c.route.lazy)) L = !1;
      ...
      if (v) {
        let R = E.findIndex((S) => v[S.route.id] !== void 0);
        L = E.slice(0, R + 1).every((S) => !Gt(S.route, c, v));
      } else L = E.every((R) => !Gt(R.route, c, v));
    }
  else {
    (L = !1), (E = []);
  ...
  function W() {
    if (
      ((g = e.history.listen(({ action: c, location: v, delta: R }) => {
        if (Y) {
          Y(), (Y = void 0);
          return;
        }
  ...
  function ve() {
    g && g(),
      ...
      h.clear(),
      I && I.abort(),
      f.fetchers.forEach((c, v) => Nt(v)),
      f.blockers.forEach((c, v) => br(v));
  }
  function ee(c) {
  ...
  function He(c, v, { flushSync: R } = {}) {
    ...
    v.actionData
      ...
      : S
        ...
        : (k = null);
    let A = v.loaderData
        ? Yr(f.loaderData, v.loaderData, v.matches || [], v.errors)
        : f.loaderData,
      F = f.blockers;
    ...
    ae(
      {
        ...
        navigation: Bt,
        revalidation: "idle",
        restoreScrollPosition: Lr(c, v.matches || f.matches),
  ...
  async function pr(c, v) {
    ...
    let R = Jt(
      ...
      { path: S, submission: k, error: A } = $r(!1, R, v),
      F = f.location,
      _ = at(f.location, S, v && v.state);
    _ = { ..._, ...e.history.encodeLocation(_) };
    let O = v && v.replace != null ? v.replace : void 0,
      T = "PUSH";
    O === !0
      ? (T = "REPLACE")
      : O === !1 ||
  ...
  async function Ae(c, v, R) {
    ...
    if ((_.active && _.matches && (A = _.matches), !A)) {
      let { error: Z, notFoundMatches: X, route: re } = $t(v.pathname);
      He(
        v,
    ...
    if (
      ...
      He(v, { matches: A }, { flushSync: F });
      return;
    }
    I = new AbortController();
    let O = Ye(e.history, v, I.signal, R && R.submission),
      T;
    if (R && R.pendingError)
      T = [Ie(A).route.id, { type: "error", error: R.pendingError }];
    else if (R && R.submission && he(R.submission.formMethod)) {
      let Z = await Un(O, v, R.submission, A, _.active, {
        replace: R.replace,
        flushSync: F,
      });
    ...
    j ||
      ((I = null),
      He(v, { matches: z || A, ...Wr(T), loaderData: Q, errors: oe }));
  }
  async function Un(c, v, R, S, k, A = {}) {
    At();
    let F = vo(v, R);
    if ((ae({ navigation: F }, { flushSync: A.flushSync === !0 }), k)) {
      let T = await dt(S, v.pathname, c.signal);
      ...
      if (T.type === "error") {
        ...
        let { notFoundMatches: j, error: z, route: Q } = $t(v.pathname);
        return {
          matches: j,
  ...
  async function Hn(c, v, R, S, k, A, F, _, O, T, j) {
    let z = k || zt(v, A),
      Q = A || F || Gr(z),
      oe = !pe && !O;
    if (S) {
      ...
      let G = await dt(R, v.pathname, c.signal);
      ...
      if (G.type === "error") {
        ...
        let { error: ie, notFoundMatches: mt, route: qe } = $t(v.pathname);
        return { matches: mt, loaderData: {}, errors: { [qe.id]: ie } };
      }
    }
    let Z = i || o,
      [X, re] = Fr(e.history, f, R, Q, v, O === !0, ye, Ue, N, m, fe, Z, l, j);
    ...
    let { loaderResults: Xe, fetcherResults: xe } = await vr(f, R, X, re, c);
    if (c.signal.aborted) return { shortCircuited: !0 };
    I && I.signal.removeEventListener("abort", Be),
      re.forEach((G) => q.delete(G.key));
    let ge = yt(Xe);
    if (ge)
      return await Ne(c, ge.result, !0, { replace: _ }), { shortCircuited: !0 };
    if (((ge = yt(xe)), ge))
      return (
        fe.add(ge.key),
        await Ne(c, ge.result, !0, { replace: _ }),
        { shortCircuited: !0 }
      );
  ...
  function Bn(c) {
    return (
      c.forEach((v) => {
        let R = f.fetchers.get(v.key),
          S = tt(void 0, R ? R.data : void 0);
        f.fetchers.set(v.key, S);
      }),
      new Map(f.fetchers)
    );
  }
  async function zn(c, v, R, S) {
    De(c);
    let k = (S && S.flushSync) === !0,
      A = i || o,
      F = Jt(f.location, f.matches, l, R, v, S == null ? void 0 : S.relative),
      _ = Le(A, F, l),
      O = ct(_, A, F);
    if ((O.active && O.matches && (_ = O.matches), !_)) {
      Se(c, v, ue(404, { pathname: F }), { flushSync: k });
      return;
    }
    let { path: T, submission: j, error: z } = $r(!0, F, S);
    if (z) {
      Se(c, v, z, { flushSync: k });
      return;
    }
    let Q = nt(_, T),
      oe = (S && S.preventScrollReset) === !0;
    if (j && he(j.formMethod)) {
      await Yn(c, v, T, Q, _, O.active, k, oe, j);
      return;
    }
    m.set(c, { routeId: v, path: T }),
      await Wn(c, v, T, Q, _, O.active, k, oe, j);
  }
  async function Yn(c, v, R, S, k, A, F, _, O) {
    At(), m.delete(c);
    function T(te) {
      if (!te.route.action && !te.route.lazy) {
        let ze = ue(405, { method: O.formMethod, pathname: R, routeId: v });
        return Se(c, v, ze, { flushSync: F }), !0;
      }
      return !1;
    }
    if (!A && T(S)) return;
    let j = f.fetchers.get(c);
    Te(c, go(O, j), { flushSync: F });
    ...
    if (A) {
      ...
      if (te.type === "error") {
        Se(c, v, te.error, { flushSync: F });
        return;
      } else if (te.matches) {
        if (((k = te.matches), (S = nt(k, R)), T(S))) return;
      } else {
        Se(c, v, ue(404, { pathname: R }), { flushSync: F });
        return;
      }
    }
    q.set(c, z);
    ...
    let { loaderResults: ht, fetcherResults: G } = await vr(f, xe, Ke, $e, Be);
    ...
    Rr(ge),
      f.navigation.state === "loading" && ge > Oe
  ...
  async function Wn(c, v, R, S, k, A, F, _, O) {
  ...
  async function Ne(
    ...
    v.response.headers.has("X-Remix-Revalidate") && (ye = !0);
    ...
    if (r) {
      let Z = !1;
      if (v.response.headers.has("X-Remix-Reload-Document")) Z = !0;
      else if (rr.test(_)) {
    ...
    I = null;
    let T =
        F === !0 || v.response.headers.has("X-Remix-Replace")
          ? "REPLACE"
          : "PUSH",
      { formMethod: j, formAction: z, formEncType: Q } = f.navigation;
    !S && !k && j && z && Q && (S = Gr(f.navigation));
    let oe = S || k;
    if (eo.has(v.response.status) && oe && he(oe.formMethod))
      await Ae(T, O, {
  ...
  async function Ge(c, v, R, S, k, A) {
    let F,
      _ = {};
    try {
      F = await so(s, c, v, R, S, k, A, a, n);
    } catch (O) {
      return (
        S.forEach((T) => {
  ...
  async function vr(c, v, R, S, k) {
    let A = Ge("loader", c, k, R, v, null),
      F = Promise.all(
        S.map(async (T) => {
  ...
  function Nt(c) {
    let v = f.fetchers.get(c);
    q.has(c) && !(v && v.state === "loading" && be.has(c)) && De(c),
      m.delete(c),
      be.delete(c),
      fe.delete(c),
      N.delete(c),
  ...
  function Vn(c) {
    let v = (C.get(c) || 0) - 1;
    v <= 0 ? (C.delete(c), N.add(c)) : C.set(c, v),
      ae({ fetchers: new Map(f.fetchers) });
  }
  function De(c) {
    let v = q.get(c);
    v && (v.abort(), q.delete(c));
  }
  function wr(c) {
    for (let v of c) {
      let R = gr(v),
  ...
  function Er() {
    ...
    for (let R of fe) {
      ...
      V(S, `Expected fetcher: ${R}`),
        S.state === "loading" && (fe.delete(R), c.push(R), (v = !0));
    }
    return wr(c), v;
  }
  function Rr(c) {
    ...
    for (let [R, S] of be)
      if (S < c) {
        ...
        V(k, `Expected fetcher: ${R}`),
          k.state === "loading" && (De(R), be.delete(R), v.push(R));
      }
    return wr(v), v.length > 0;
  }
  function Jn(c, v) {
    let R = f.blockers.get(c) || et;
    return $.get(c) !== v && $.set(c, v), R;
  }
  function br(c) {
  ...
  function ut(c, v) {
    let R = f.blockers.get(c) || et;
    V(
      (R.state === "unblocked" && v.state === "blocked") ||
        (R.state === "blocked" && v.state === "blocked") ||
        (R.state === "blocked" && v.state === "proceeding") ||
        (R.state === "blocked" && v.state === "unblocked") ||
        (R.state === "proceeding" && v.state === "unblocked"),
      `Invalid blocker state transition: ${R.state} -> ${v.state}`,
    );
    let S = new Map(f.blockers);
    S.set(c, v), ae({ blockers: S });
  }
  function Sr({ currentLocation: c, nextLocation: v, historyAction: R }) {
    if ($.size === 0) return;
    ...
    if (
      ...
      A({ currentLocation: c, nextLocation: v, historyAction: R })
    )
      return k;
  }
  function $t(c) {
    let v = ue(404, { pathname: c }),
      R = i || o,
      { matches: S, route: k } = Vr(R);
    return { notFoundMatches: S, route: k, error: v };
  }
  function Gn(c, v, R) {
    if (((y = c), (b = v), (w = R || null), !x && f.navigation === Bt)) {
      x = !0;
    ...
    if (u)
      if (c) {
        if (Object.keys(c[0].params).length > 0)
          return { active: !0, matches: Rt(v, R, l, !0) };
      } else return { active: !0, matches: Rt(v, R, l, !0) || [] };
    return { active: !1, matches: null };
  }
  async function dt(c, v, R) {
  ...
  if (
    ...
    if (l.route.index && !u)
      ...
      let p = new URLSearchParams(s.search),
        g = p.getAll("index");
      p.delete("index"),
        g.filter((y) => y).forEach((y) => p.append("index", y));
      let h = p.toString();
      s.search = h ? `?${h}` : "";
    }
  }
...
function Fr(e, t, r, n, a, o, i, l, s, u, p, g, h, y) {
  ...
  try {
    ...
    if (n)
      if (u) {
        ...
        throw ue(405, { method: t.method, pathname: g, routeId: r.route.id });
      } else return { type: "data", result: void 0 };
    else if (u) i = await s(u);
    else {
      let p = new URL(t.url),
        g = p.pathname + p.search;
      throw ue(404, { pathname: g });
    }
  } catch (u) {
...
function zr(e, t, r, n, a, o) {
  ...
  return (
    a.forEach((s) => {
      let { key: u, match: p, controller: g } = s,
        h = o[u];
      if (
...
function Oo(e, t = [], r = null, n = null) {
  ...
  return a.reduceRight((s, u, p) => {
    let g,
      ...
      w = null;
    r &&
      ((g = o && u.route.id ? o[u.route.id] : void 0),
    ...
    b = d.useCallback(
      (L, { deletedFetchers: D, flushSync: f, viewTransitionOpts: M }) => {
        ...
        if (t && f) {
          ...
          I.finished.finally(() => {
            t(() => {
              u(void 0), g(void 0), o(void 0), l({ isTransitioning: !1 });
            });
          }),
            t(() => g(I));
          return;
  ...
  let {
    ...
    y = d.useMemo(() => {
      ...
      return w == null
        ? null
        : {
            location: { pathname: w, search: u, hash: p, state: g, key: h },
            navigationType: n,
          };
    }, [i, s, u, p, g, h, n]);
  return (
...
function ai() {
  ...
  return (gt = e.relList.supports("preload")), (e = null), gt;
}
function Zr(e) {
  return { __html: e };
}
      ...
      a.map(async (w, b) =>
        w.resolve(async (x) => {
          ...
          if (E && E.hasClientLoader) {
            E.hasLoader && (i = !0);
            try {
              let L = await Cn(x, p, g, w.route.id);
    ...
    function a(p) {
      let g =
        p.tagName === "FORM"
          ...
          : p.getAttribute("href");
      if (!g) return;
      let h = new URL(g, window.location.origin);
      Tt.has(h.pathname) || xt.add(h.pathname);
    }
    async function o() {
      let p = Array.from(xt.keys()).filter((g) =>
        Tt.has(g) ? (xt.delete(g), !1) : !0,
      );
      if (p.length !== 0)
        try {
...
async function _n(e, t, r, n, a, o) {
  ...
  let u = await s.json(),
    ...
    g = Object.values(u).reduce(
      (y, w) => (w && !p.has(w.id) && (y[w.id] = w), y),
      {},
    );
  Object.assign(t.routes, g), e.forEach((y) => vi(y, Tt));
  let h = new Set();
  Object.values(g).forEach((y) => {
    y && (!y.parentId || !g[y.parentId]) && h.add(y.parentId);
  }),
    h.forEach((y) => o(y || null, dr(g, r, null, n, y)));
}
function vi(e, t) {
  if (t.size >= mi) {
...
To reference the meta function API, see https://remix.run/route/meta`,
  ...
  return d.createElement(
    ...
    s.flat().map((g) => {
      ...
      if ("title" in g)
        return d.createElement("title", { key: "title" }, String(g.title));
      if (
        ("charset" in g &&
          (g.charSet ?? (g.charSet = g.charset), delete g.charset),
        "charSet" in g && g.charSet != null)
      )
        return typeof g.charSet == "string"
          ? d.createElement("meta", { key: "charSet", charSet: g.charSet })
          : null;
      if ("script:ld+json" in g)
        try {
          let h = JSON.stringify(g["script:ld+json"]);
      ...
      return d.createElement("meta", { key: JSON.stringify(g), ...g });
    }),
  );
}
function bi(e) {
...
try {
  Nn && (window.__reactRouterVersion = "7.1.0");
} catch {}
var $n = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i,
...
var Ci = d.forwardRef(function (
  ...
  let g = it(i, { relative: u.relative }),
    h = Ee(),
    y = d.useContext(Ve),
    { navigator: w, basename: b } = d.useContext(de),
    x = y != null && Ii(g) && l === !0,
    E = w.encodeLocation ? w.encodeLocation(g).pathname : g.pathname,
    P = h.pathname,
    L =
...
function Ti({ getKey: e, storageKey: t, ...r }) {
  ...
  let s = ((u, p) => {
    if (!window.history.state || !window.history.state.key) {
      let g = Math.random().toString(32).slice(2);
      window.history.replaceState({ key: g }, "");
    }
    try {
      ...
      console.error(g), sessionStorage.removeItem(u);
    }
  }).toString();
  return d.createElement("script", {
    ...r,
...
function Ai(e, { relative: t } = {}) {
  ...
  if (e == null) {
    ...
    if (s.some((p) => p === "")) {
      l.delete("index"),
        s.filter((g) => g).forEach((g) => l.append("index", g));
      let p = l.toString();
      o.search = p ? `?${p}` : "";
    }
  }
